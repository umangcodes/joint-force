...
class Employee():
    def __init__(self, name, start_time, end_time, location, actual_start_time="NA", actual_end_time="NA"):
        ...
        self.break_overflow = []
        self.remark = {"start_status": 0, "end_status": 0, "special": 0}
        """
        ...
        try:
            if time_difference > datetime.timedelta(hours=global_logic_flags.Regulations_variables.max_shift_time):
                self.shift_type = "max time"
                    ...
                    hours=global_logic_flags.Regulations_variables.full_shift_time_difference):
                self.shift_type = "full time"
            elif time_difference >= datetime.timedelta(
                    hours=global_logic_flags.Regulations_variables.half_shift_time_difference):
    ...
    def estimate_break_time(self):
        if self.punch_status == "NOT SIGNED IN":
            if self.shift_type == "max time":  # TODO: another if loop which will check if the actual time is available or not and if not then calculate using admin data start time
                ...
                self.break_est_3 = self.break_est_2 + datetime.timedelta(hours=2)
            elif self.shift_type == "full time":  # TODO: another if loop which will check if the actual time is available or not and if not then calculate using admin data start time
                self.break_est_1 = datetime.timedelta(hours=2)
                self.break_est_1 = self.start_time + self.break_est_1  # TODO: Fault....function present to add self.actual_start_time also contains date.
                self.break_est_long = self.break_est_1 + datetime.timedelta(hours=2)
                self.break_est_2 = self.break_est_long + datetime.timedelta(hours=2, minutes=30)
            elif self.shift_type == "half time":
                self.break_est_1 = datetime.timedelta(hours=2, minutes=00)
                ...
                print(
                    f"ERROR: cannot find value of <object> attribute shift_type\n\t\t\t|object.shift_type = {self.punch_status}| ")
    ...
    def estimate_break_time_with_dict(self):
        if self.punch_status == "NOT SIGNED IN":
            if self.shift_type == "full time":  # TODO: another if loop which will check if the actual time is available or not and if not then calculate using admin data start time
                ...
                estimate_break_time_dict["break_estimate_1"] = self.start_time + estimate_break_time_dict[
                    "break_estimate_1"]  # TODO: Fault....function present to add self.actual_start_time also contains date.
                estimate_break_time_dict["break_estimate_long"] = estimate_break_time_dict[
                                                                      "break_estimate_1"] + datetime.timedelta(hours=2)
                estimate_break_time_dict["break_estimate_2"] = estimate_break_time_dict[
                                                                   "break_estimate_long"] + datetime.timedelta(hours=2,
                                                                                                               minutes=30)
            elif self.shift_type == "half time":
                estimate_break_time_dict = self.break_info[1]
            ...
            if self.shift_type == "full time":  # TODO: another if loop which will check if the actual time is available or not and if not then calculate using admin data start time
                estimate_break_time_dict = self.break_info[1]
                estimate_break_time_dict["break_estimate_1"] = datetime.timedelta(hours=2)
                estimate_break_time_dict["break_estimate_1"] = self.actual_start_time + estimate_break_time_dict[
                    "break_estimate_1"]  # TODO: Fault....function present to add self.actual_start_time also contains date.
                estimate_break_time_dict["break_estimate_long"] = estimate_break_time_dict[
                                                                      "break_estimate_1"] + datetime.timedelta(hours=2)
                estimate_break_time_dict["break_estimate_2"] = estimate_break_time_dict[
